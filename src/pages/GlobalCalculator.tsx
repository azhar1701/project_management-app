...
import { calculatePeakDischarge } from '../utils/calculations';
...
export function GlobalCalculator() {
  ...
  // Calculator State
  const [c, setC] = useState<number>(0.75);
  const [i, setI] = useState<number>(120);
  const [a, setA] = useState<number>(45.5);

  // Delegate math to pure utility function for testability
  const q = calculatePeakDischarge(c, i, a);
        ...
        <div className="w-full lg:w-1/3 bg-paper tech-border tech-shadow flex flex-col shrink-0">
          ...
          <div className="p-6 space-y-6 flex-1 overflow-y-auto">
            ...
            <div className="pt-6 border-t border-ink">
              <div className="bg-surface p-6 tech-border tech-shadow transform transition-all">
                <p className="text-[10px] font-mono font-bold uppercase tracking-widest text-accent mb-2">Peak Discharge (Q)</p>
                <p className="text-3xl font-mono font-bold text-ink">{q} <span className="text-sm font-normal text-muted">m³/s</span></p>
              </div>
            </div>
